...
//         std::uniform_int_distribution<uint32_t> dist(0, next_hops.size() - 1);
//         idx = dist(gen);
...
//         }
//     } 
//     std::cout << "Node " << m_node_id << " forwarding packet to target " << target_node_id 
//               << " via next hop index " << idx 
//               << " out of size" << next_hops.size()
//               << " (next hop: " << std::get<0>(next_hops[idx]) << ", "
//               << std::get<1>(next_hops[idx]) << ", "
//               << std::get<2>(next_hops[idx]) << ")"
