...
def calculate_fstate_shortest_path_without_gs_relaying_downlink_uplink(
        output_dynamic_state_dir,
        time_since_epoch_ns,
        num_satellites,
        num_ground_stations,
...
def calculate_fstate_shortest_path_without_gs_relaying_downlink_uplink_lasthop_splitting(
        output_dynamic_state_dir,
        time_since_epoch_ns,
        num_satellites,
        num_ground_stations,
            ...
            for dst_gid in range(num_ground_stations):
                ...
                if len(possibilities) > 0:
                    ...
                    if curr != dst_sat:
                        ...
                        # This is the destination satellite, the next hop can be the ground station or near by satellite that reachees the ground stations
                        for candidate_satellite in possible_dst_sats:
                            if candidate_satellite[1] in sat_net_graph_only_satellites_with_isls.neighbors(curr):
                                next_hop_decision = next_hop_decision +(
                                    candidate_satellite[1],
                                    sat_neighbor_to_if[(curr, candidate_satellite[1])],
                                    sat_neighbor_to_if[(candidate_satellite[1], curr)]
                                )
                ...
                #     ))
                if not prev_fstate or prev_fstate[(curr, dst_gs_node_id)] != next_hop_decision:
                    line = [str(curr), str(dst_gs_node_id)] + [str(x) for x in next_hop_decision]
...
def calculate_fstate_shortest_path_without_gs_relaying_downlink_uplink_equal(
        output_dynamic_state_dir,
        time_since_epoch_ns,
        num_satellites,
        num_ground_stations,
